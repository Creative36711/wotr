...
- **Повторный бой после аварийного выхода снова читается.** Исход боя пишется в `temp/rts_battle_result_<конфликт>.json`, и этот файл переживал запуски: после выхода по Ctrl в нём оставалась запись `ABORTED`, новый запуск боя её не удалял, и watcher интерфейса через несколько секунд принимал старый «аварийный» исход за исход новой попытки, показывал сообщение и останавливался — реальный результат, который игра потом фиксировала на экране статистики, никто не ждал. Теперь каждый запуск начинается с чистого листа: Rust удаляет файл исхода перед стартом BFME, а watcher вдобавок игнорирует записи, сделанные раньше момента запуска попытки (страховка от гонки). Повторный бой после Ctrl завершается как обычный: победитель из экрана статистики применяется к конфликту.
- **В именах снимков и в отчётах — реальное разрешение рабочего стола, а не запись из реестра.** Имя `score-chart-1280x720@640x480.png` читалось так: кадр 1280×720 (размер окна игры) на «рабочем столе» 640×480 — хотя монитор 1920×1080. Разрешение бралось из сохранённого режима дисплея в реестре Windows (`ENUM_REGISTRY_SETTINGS`), а реестр на некоторых машинах хранит устаревшую запись (например, 640×480 с какого-то старого сеанса). Теперь разрешение рабочего стола снимается в момент запуска боя, **до старта BFME**, когда текущий режим дисплея гарантированно равен рабочему столу; этот снимок используется и в имени файла (`…-1280x720@1920x1080.png`), и в `environment.json`/`analysis.json`. Реестровый режим остался только запасным путём для кода вне потока запуска боя. Уменьшение PNG по-прежнему отдельно (не больше 1280×720) и на запись разрешения не влияет.
- **Совместимость:** версия приложения 0.52.4. Версии форматов прежние: `world.json` 45, `roster.json` 17, `savegame.json` 62.

## Обновление 0.52.3

- **Мост снова компилируется под Windows.** В `DisplayDeviceW` (перечисление дисплеев для диагностики) выводился `Default`, но три его поля — массивы `[u16; 128]` — трейт `Default` не реализуют (в стандартной библиотеке он есть только для массивов до 32 элементов), из-за чего `tauri build` падал с `E0277`. Структура и так инициализируется только через `zeroed()`, которому трейт не нужен, — `Default` из производных убран. Раскладка и размер (`cb` + имя + строка + флаги + id, 840 байт) не менялись.
- **Совместимость:** версия приложения 0.52.3 (0.52.2 не собирался под Windows и в релиз не вышел). Версии форматов прежние: `world.json` 45, `roster.json` 17, `savegame.json` 62.

## Обновление 0.52.2

- **У капитанов появилась собственная заглушка портрета.** У всех семи типов капитанов встроенного мода портрет пуст (`portrait: ""`), и интерфейс показывал чужую заглушку героя — в карточке командира армии и в редакторе типа капитана, — а в списках потерь автобоя капитан оставался вообще без картинки. Добавлен ассет `public/assets/ui/captain-placeholder.jpg` в стиле заглушек героев и юнитов; заглушка выбирается по роду командира: герой → геройская, капитан → капитанская. Проводка: `data-kind="captain"` у `.leader-portrait` (инспектор армии), класс `.battle-loss-portrait.captain` (отчёт автобоя), переопределение фона `.fake-portrait` в капитанском редакторе (`captains.css`). Реальный портрет типа капитана, если он задан, по-прежнему перекрывает заглушку.
- **Стартовые армии «Vanilla 2.01» стоят на своих объектах из коробки.** Четыре стартовые армии встроенного мода лежали мимо своих объектов, и редактор/инспектор честно предупреждал «Армия находится вне своего объекта»: «Host: Elrond» стоял в Тролльей пустоши вместо Ривенделла (14:14), «Host of Captain Shagrat» — в Литлад вместо Барад-дура (21:24), «Host: Witch-king» (Ангмар) — в предгорьях вместо Карн Дума (12:10), «Host: Haldar» — в Северных пустошах вместо Рудаура. Армии возвращены на якорные гексы своих объектов. Рудаур числился нейтральным (`side: civilian`), хотя по всем данным это объект ангмара (культура, отряды Rhudaur Spearmen, герой Халдар с точкой призыва «rhudaur», карта BFME `map wor ang rhudaur`): объект переведён на ангмар, а осиротевшие ссылки кампании и ростера — состояние локации и точка призыва Халдара, хранившие старый id «rhudaur» — переведены на актуальный id объекта.
- **Защитник крепости больше не обязан быть первым слотом.** Первый слот комнаты BFME всегда игрок, и игрок может атаковать, пока крепость обороняет бот. Расстановка флажков в комнате переписана: вместо «владелец крепости кликается первым, N кликов в главную точку» — ровно один клик на слот по возрастанию номеров (комната ставит флажки строго по номерам слотов). Старый приём при владельце в слоте 2+ укладывал на главную точку крепости и флажок атакующего — противник появлялся внутри крепости (жалоба «Ортханк: противник заспавнился внутри»). Порты обновлены в Rust (`assign_start_positions`) и в эталонном прототипе (`room.py` + его тест). Дополнительно «крепостной бой» теперь определяется не стратегическим типом «осада», а данными карты: если у места боя задана главная позиция защитника (`rtsFortress.defenderStartPosition` + `fortressDefenseIndex`), владелец крепости закрепляется на ней независимо от типа. Это чинит бои за Минас Тирит (столица, стратегический тип «поселение», но карта BFME крепостная), где защитник раньше вставал на случайную точку обороны, а в конфигурацию боя не попадала точка защитника; Каир Андрос (осада без крепостной точки) по-прежнему расставляется как обычная локация.
- **«Тест координат» расставляет точки последовательно.** Раньше тест перемешивал точки обороны и атаки, и повторные запуски давали разную расстановку. Теперь защитники получают точки обороны в порядке калибровки (владелец крепости — выбранную главную `defense[fortressDefenseIndex]`), атакующие — точки атаки по порядку. Тест воспроизводим: две попытки дают одинаковую расстановку, и каждую точку можно сверить со снимком комнаты.
- **В диагностике пишется реальное разрешение монитора.** Полноэкранный BFME может временно переключить режим дисплея (например, на 1280×720), и прежний замер через `GetSystemMetrics` в этот момент показывал игровое разрешение, а не монитор пользователя: `environment.json`, `analysis.json` и имена PNG утверждали «1280x720» при мониторе 1920×1080. Разрешения теперь читаются из сохранённого режима рабочего стола (`EnumDisplaySettingsW` с `ENUM_REGISTRY_SETTINGS`; для остальных мониторов — текущий режим, полноэкранная игра переключает только основной): в отчёте будет 1920×1080, а игровой режим виден по `gameWindow` и размеру кадра на снимках. Имя файла при расхождении: `…-1280x720@1920x1080.png`. Уменьшение PNG ради экономии места на запись разрешения не влияет.
- **Совместимость:** версия приложения 0.52.2 (0.52.1 в историю кода не входил — это был импорт трёх боевых диагностик без изменения кода и версии). Модель данных не менялась — версии форматов прежние (`world.json` 45, `roster.json` 17, `savegame.json` 62); данные встроенного мода (стартовые армии, сторона Рудаура) исправлены, и по правилу проекта сохранение совместимо только с той же версией приложения, так что старые кампании всё равно не переносятся.

## Обновление 0.52.0

- **Проходимость видна при планировании движения.** Когда выделена своя армия, способная двигаться (тактический вид), карта подсвечивает проходимость по всей видимой области: **красным** — гексы, в которые армия не может войти вовсе (вода, закрытые перевалы, запретительная стоимость входа); **жёлтым** — местность дороже стандартного хода (лес, болото, холмы, горы, река без моста); **тёмным** — проходимые гексы, до которых армия не дотянется за оставшиеся очки движения. Гора без перевала больше не выглядит как холм: игрок видит, почему маршрут уходит в обход, ещё до отдачи приказа. Подсветка — инструмент планирования, а не постоянный слой: она исчезает при снятии выделения (Esc, клик по пустому месту), не показывается для чужих армий и после отдачи приказа, а гексы в тумане войны не подсвечиваются вовсе, чтобы подсветка не выдавала рельеф. Классификация переиспользует ту же стоимость входа (`cellMovementCost` в `src/hex/hexGrid.ts`), по которой строятся маршруты и приказы, поэтому карта не может разойтись с реальной механикой движения. Новый модуль `src/hex/movementOverlay.ts` — чистая функция: категории гекса и BFS досягаемости через `findReachable`; слой `movement-terrain-layer` рисуется между тактической сеткой (z3) и туманом войны (z5), под маркерами и стрелками приказов.
- **Легенда цветов** появляется у панели тактики, пока подсветка активна: «Непроходимо», «Замедляет», «Вне досягаемости». Три новых строки получили перевод в `UI_EN`, английский интерфейс остался без кириллицы.
- **Харнесс дополнен 37 проверками.** Классификация на синтетических гексах (вода, лес, равнина, дорога в лесу, река, запретительная стоимость) и сборка групп на мини-карте (бюджет хода, туман, вражеские стоп-гексы, нуль очков движения) — как чистая функция; затем DOM-проверки слоя на настоящей карте: появление при выделении своей армии, исчезновение при снятии выделения, при выделении чужой армии и после отдачи приказа, согласованность каждой цветовой группы с классификацией, недостижимость затемнённых гексов и неподсвечивание тумана и гекса самой армии. Сейчас: 68 проверок, 0 провалено.
- **Совместимость:** версия приложения 0.52.0. Механика чисто визуальная и не меняет модели данных — версии форматов не поднимались (`world.json` 45, `roster.json` 17, `savegame.json` 62), но по правилу проекта сохранение совместимо только с той же версией приложения, так что старые кампании всё равно не переносятся.
...
- **Растянутые галочки устранены.** Прошлая правка дала чекбоксам `accent-color`, но не размер: правило `.economy-type-card > label input` и `.economy-type-grid input`, написанное для текстовых полей (`width:100%; height:30px`), растягивало чекбокс «Сигнальный огонь» во вкладке «Постройки» и «Ковка включена» во вкладке «Кольцо» в пустой белый квадрат. Правило больше не касается `[type='checkbox']` и `[type='radio']`, а галочки и радио получили глобальный размер 13 px. Замер после правки: «Сигнальный огонь» — 13×13 px, `accent-color rgb(109, 143, 90)`.
- **Проверка `verify/styles.mjs` дополнена критерием растянутой галочки** (размер больше 20 px), поэтому такой баг больше не пройдёт незамеченным: прошлый прогон с `accent-color` его пропустил. Сейчас: 15 экранов, 5335 контролов, 0 нарушений.
