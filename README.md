...
- **Версия:** 0.52.6 · форматы данных: `world.json` 45, `roster.json` 17, `savegame.json` 62.
...
5. **Загрузочный экран (маска).** Пока мост настраивает бой, экран занимает заставка места боя в духе оригинальной War of the Ring: картинка локации (или заглушка), её название, текущий шаг и полоса прогресса (`BattleLoadingScreen` + `src/battleLoading.ts`). Это **отдельное topmost-окно Tauri** («battle-mask», fullscreen borderless, без панели задач): Rust показывает его перед запуском автоматизации и вешает Win32-стили сквозного окна `WS_EX_TRANSPARENT | WS_EX_LAYERED | WS_EX_NOACTIVATE | WS_EX_TOPMOST` — оно непрозрачное визуально, но пропускает клики `SendInput` в игру и не крадёт фокус. Чтобы игру можно было спрятать под заставкой, **бой стартует в оконном режиме** на разрешение рабочего стола (`-win`), а после клика «Начать игру» мост сам переводит игру в fullscreen (Alt+Enter) — маска уходит, игрок берёт управление. Прогресс каждой фазы мост пишет в `temp/rts_progress.json`, интерфейс опрашивает его, поэтому заставка живёт от «Генерация армий… 5 %» до «Бой начинается! 100 %», а не застывает на «Запуск игры…». Запрос UAC при этом никуда не делся: это защищённый рабочий стол Windows, его нельзя ни убрать, ни перекрыть — но маска остаётся на месте до и после него. Когда детектор находит экран статистики, маска возвращается на пару секунд в режиме «Обработка результата…», пока исход применяется к конфликту.
...
- Окно UAC «Разрешить этому приложению вносить изменения…» появляется на каждый запуск боя и не может быть убрано: это защищённый рабочий стол Windows (приложению нужны права на запись BIG-файлов в папку игры и на ввод для `game.dat`). Всё, что можно было сделать, — спрятать игру под заставкой до и после запроса.
