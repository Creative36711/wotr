...
- **Версия:** 0.51.9 · форматы данных: `world.json` 45, `roster.json` 17, `savegame.json` 62.
...
Тема интерфейса тёмная, а браузерные контролы светлые: кнопка или поле, до которых не дошло ни одно CSS-правило, выглядят белой заплаткой. Поэтому у каждого контрола должно быть своё правило (галочки и радио красятся глобально через `accent-color`). За этим следит `node verify/styles.mjs`: он рендерит настоящие экраны в jsdom и сравнивает вычисленные стили с «голым» контролом.
