...
- **Версия:** 0.52.5 · форматы данных: `world.json` 45, `roster.json` 17, `savegame.json` 62.
...
5. **Загрузочный экран (маска).** Пока мост настраивает бой, интерфейс показывает заставку места боя в духе оригинальной War of the Ring: картинка локации (или заглушка), её название, текущий шаг и полоса прогресса (`BattleLoadingScreen` + `src/battleLoading.ts`). Экран чисто информационный: клики сквозь него уходят в игру, курсор спрятан, фокус он не перехватывает — визуальную сторону дополняют Win32-стили сквозного окна (WS_EX_TRANSPARENT | WS_EX_LAYERED | WS_EX_NOACTIVATE | WS_EX_TOPMOST) на стороне Rust. Когда бой начался, маска уходит; когда детектор находит экран статистики, она возвращается на пару секунд в режиме «Обработка результата…», пока исход применяется к конфликту.
