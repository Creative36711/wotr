...
def assign_start_positions(slot_positions, fortress_owner=None):
    ...
    Механика: комната расставляет флажки строго по номерам слотов — каждый
    клик по миникарте ставит флажок СЛЕДУЮЩЕМУ слоту (1, 2, 3, ...). Поэтому
    каждому слоту нужен ровно 1 клик в его точку, по возрастанию номеров.

    Владелец крепости НЕ обязан быть слотом 1: первый слот всегда игрок,
    и он может атаковать, пока крепость обороняет бот. Раньше владельца
    кликали первым N раз (N = номер слота) в главную точку «на заполнение»
    предыдущих слотов — при владельце в слоте 2+ флажок атакующего тоже
    оказывался на главной точке крепости (противник спавнился внутри).
    Один клик на слот в порядке возрастания ставит каждый флажок в свою
    точку, а клик владельца ложится на главную точку крепости.
    """
    if fortress_owner and fortress_owner in slot_positions:
        log(f"[room] стартовая позиция: владелец крепости — слот {fortress_owner}")
