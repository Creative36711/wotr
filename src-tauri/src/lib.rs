...
// default MapCache are embedded into the executable and extracted into
// portable_data on the first launch (or whenever the files are missing).
const DEFAULT_MODULE_PATCH201: &[u8] = include_bytes!("../assets/bfme201/_patch201ini.big");
const DEFAULT_MAPS_BIG: &[u8] = include_bytes!("../assets/bfme201/__wotr_maps.big");
const DEFAULT_MODULE_PATCH201_SIZE: u64 = DEFAULT_MODULE_PATCH201.len() as u64;
const DEFAULT_MAPS_BIG_SIZE: u64 = DEFAULT_MAPS_BIG.len() as u64;

// Generated by build.rs from public/mods/default/rts/map-caches/locations/.
include!(concat!(env!("OUT_DIR"), "/embedded_map_caches.rs"));
...
fn initialize_mods(app:&AppHandle)->Result<(),String>{
 ...
 // Only the obsolete module name from earlier builds is removed; the real
 // 2.01 archives and MapCaches below are always restored automatically.
 for legacy in ["rts/modules/wotr-ini.big"]{let path=default.join(legacy);if path.exists(){let _=fs::remove_file(path);}}
 ensure_default_rts_assets(&default)?;
 let app_file=root.join("app.json");if !app_file.exists(){atomic_write(&app_file,DEFAULT_APP.as_bytes())?}Ok(())
}

/// Extract the embedded 2.01 archives and MapCaches into the default mod and
/// keep its mod.json RTS entries filled. Works fully offline.
fn ensure_default_rts_assets(default:&Path)->Result<(),String>{
 let module_path=default.join("rts/modules/patch201ini.big");
 if !module_path.exists(){atomic_write(&module_path,DEFAULT_MODULE_PATCH201)?}
 let maps_path=default.join("rts/maps/maps.big");
 if !maps_path.exists(){atomic_write(&maps_path,DEFAULT_MAPS_BIG)?}
 for (location_id,bytes) in EMBEDDED_MAP_CACHES{
  let cache_path=default.join("rts/map-caches/locations").join(format!("{location_id}.big"));
  if !cache_path.exists(){atomic_write(&cache_path,bytes)?}
 }
 // Keep the shipped mod.json metadata in sync when entries were cleared.
 let meta_path=default.join("mod.json");
 let mut meta:Value=serde_json::from_str(&fs::read_to_string(&meta_path).map_err(|e|e.to_string())?).map_err(|e|e.to_string())?;
 let mut rts=meta.get("rts").cloned().unwrap_or_else(||json!({"enabled":true,"factionOrder":[],"moduleFiles":[],"mapsFile":null,"mapCacheTargetFileName":"__wotr_maps_cache.big","networkRules":"0 0 0 400 1000 -1 -1 -1 -1 -1"}));
 let mut changed=false;
 let module_entry=json!({"id":"patch201ini","originalFileName":"_patch201ini.big","targetFileName":"_patch201ini.big","storageName":"rts/modules/patch201ini.big","size":DEFAULT_MODULE_PATCH201_SIZE});
 if rts.get("moduleFiles").and_then(Value::as_array).map_or(true,|files|files.is_empty()){if let Some(object)=rts.as_object_mut(){object.insert("moduleFiles".into(),json!([module_entry]));changed=true}}
 let maps_entry=json!({"id":"maps","originalFileName":"__wotr_maps.big","targetFileName":"__wotr_maps.big","storageName":"rts/maps/maps.big","size":DEFAULT_MAPS_BIG_SIZE});
 if rts.get("mapsFile").map(|value|value.is_null()).unwrap_or(true){if let Some(object)=rts.as_object_mut(){object.insert("mapsFile".into(),maps_entry);changed=true}}
 if changed{
  if let Some(object)=meta.as_object_mut(){object.insert("rts".into(),rts);}
  atomic_write(&meta_path,serde_json::to_string_pretty(&meta).map_err(|e|e.to_string())?.as_bytes())?;
 }
 Ok(())
}
...
fn battle_result_path(temp:&Path,conflict_id:&str)->PathBuf{
