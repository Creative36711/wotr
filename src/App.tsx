...
import { startSessionLog } from './game/sessionLog'
...
export default function App() {
  ...
  useEffect(()=>{const handle=(event:KeyboardEvent)=>{if(appView!=='workspace'){if(event.key==='Escape'&&appView==='faction')setAppView('menu');return}const modifier=event.ctrlKey||event.metaKey;const target=event.target as HTMLElement|null;const typing=['INPUT','SELECT','TEXTAREA'].includes(target?.tagName??'');if(modifier&&event.key.toLowerCase()==='s'){event.preventDefault();void persist()}else if(modifier&&event.key.toLowerCase()==='z'&&!event.shiftKey){event.preventDefault();undo()}else if(modifier&&(event.key.toLowerCase()==='y'||event.key.toLowerCase()==='z'&&event.shiftKey)){event.preventDefault();redo()}else if(!typing&&event.key.toLowerCase()==='g'){const state=useMapStore.getState();setViewMode(state.viewMode==='strategic'?'cinematic':'strategic')}else if(!typing&&event.key.toLowerCase()==='t')setViewMode('tactical');else if(!typing&&(event.key==='Delete'||event.key==='Backspace')){const state=useMapStore.getState();const armyId=state.selectedArmyId;if(armyId&&state.campaign.pendingOrders.some((order)=>order.armyId===armyId)){event.preventDefault();state.cancelArmyOrder(armyId)}}else if(event.key==='Escape'){const state=useMapStore.getState();const closingOverlay=dataEditorOpen||state.addKind!==null||state.hexEdit;setDataEditorOpen(false);setAddKind(null);setHexEdit(false);if(!closingOverlay&&(state.selectedArmyId||state.selectedHexId)){state.selectArmy(null);state.selectHex(null)}}};window.addEventListener('keydown',handle);return()=>window.removeEventListener('keydown',handle)},[appView,dataEditorOpen,persist,redo,setAddKind,setHexEdit,setViewMode,undo])
  ...
  if(appView!=='workspace')return <MainMenu view={appView} canContinue={saveCompatible} continueReason={saveCompatibilityReason} mapImageUrl={mapImageUrl} activeModName={activeMod?.name??activeModId} onMods={()=>setAppView('mods')} onRtsSettings={()=>setAppView('rts-settings')} onLanguage={()=>setAppView('language')} onNewCampaign={()=>void handleNewCampaign()} onContinue={()=>{if(saveCompatible){setMode('game');void startSessionLog(useMapStore.getState().gameSave?.createdAt,{appVersion:GAME_VERSION,modId:activeModId,playerFactionId:useMapStore.getState().campaign.playerFactionId,round:useMapStore.getState().campaign.round});setAppView('workspace')}}} onEditor={()=>{setMode('edit');setAppView('workspace')}} onBack={()=>setAppView('menu')} onStart={(factionId,fogEnabled,activeFactionIds,strategicDifficulty,rtsDifficulty)=>{newGame(factionId,fogEnabled,activeModId,activeFactionIds,strategicDifficulty,rtsDifficulty);void startSessionLog(useMapStore.getState().gameSave?.createdAt,{appVersion:GAME_VERSION,modId:activeModId,playerFactionId:factionId,fogEnabled,activeFactionIds,strategicDifficulty,rtsDifficulty},extractSaveGame(currentWorld(),null,activeModId??'default'),true);setSaveCompatible(true);setSaveCompatibilityReason(null);setAppView('workspace')}}/>

  return <div className="app-shell"><Topbar saveState={saveState} activeModName={activeMod?.name??activeModId} onSave={()=>void persist()} onOpenData={()=>setDataEditorOpen(true)} onMenu={()=>{setDataEditorOpen(false);setAppView('menu')}}/><main className="workspace"><Sidebar onFocus={(id)=>setFocusTarget({id,nonce:Date.now()})}/><section className="map-column"><MapCanvas focusTarget={focusTarget} mapImageUrl={mapImageUrl}/></section><Inspector activeModId={activeModId} activeMod={activeMod} appSettings={appSettings} onModChange={(definition)=>void handleModDefinitionChange(definition)}/></main>{dataEditorOpen&&activeMod&&<WorldDataEditor onClose={()=>setDataEditorOpen(false)} activeMod={activeMod} onModChange={(definition)=>void handleModDefinitionChange(definition)}/>}<ConflictModal activeMod={activeMod} appSettings={appSettings}/><BattleReportModal/><BattleLoadingScreen/>{mode==='game'&&<CampaignResultModal onMenu={()=>setAppView('menu')}/>} {saveState==='error'&&<div className="save-error" role="alert"><b>Не удалось записать данные мода.</b><span>{saveErrorDetail??<>Запустите проект через <code>npm run dev</code> и повторите.</>}</span><button onClick={()=>void persist()}>Повторить</button></div>}</div>
}
