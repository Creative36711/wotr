...
export default function App() {
  ...
  return <div className="app-shell"><Topbar saveState={saveState} activeModName={activeMod?.name??activeModId} onSave={()=>void persist()} onOpenData={()=>setDataEditorOpen(true)} onMenu={()=>{setDataEditorOpen(false);setAppView('menu')}}/><main className="workspace"><Sidebar onFocus={(id)=>setFocusTarget({id,nonce:Date.now()})}/><section className="map-column"><MapCanvas focusTarget={focusTarget} mapImageUrl={mapImageUrl}/></section><Inspector activeModId={activeModId} activeMod={activeMod} onModChange={(definition)=>void handleModDefinitionChange(definition)}/></main>{dataEditorOpen&&activeMod&&<WorldDataEditor onClose={()=>setDataEditorOpen(false)} activeMod={activeMod} onModChange={(definition)=>void handleModDefinitionChange(definition)}/>}<ConflictModal activeMod={activeMod} appSettings={appSettings}/><BattleReportModal/>{mode==='game'&&<CampaignResultModal onMenu={()=>setAppView('menu')}/>} {saveState==='error'&&<div className="save-error" role="alert"><b>Не удалось записать данные мода.</b><span>{saveErrorDetail??<>Запустите проект через <code>npm run dev</code> и повторите.</>}</span><button onClick={()=>void persist()}>Повторить</button></div>}</div>
}
