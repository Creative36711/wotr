...
export default function App() {
  ...
  if(appView!=='workspace')return <MainMenu view={appView} canContinue={saveCompatible} continueReason={saveCompatibilityReason} mapImageUrl={mapImageUrl} activeModName={activeMod?.name??activeModId} onMods={()=>setAppView('mods')} onRtsSettings={()=>setAppView('rts-settings')} onLanguage={()=>setAppView('language')} onNewCampaign={()=>void handleNewCampaign()} onContinue={()=>{if(saveCompatible){setMode('game');void startSessionLog('continue',{appVersion:GAME_VERSION,modId:activeModId,playerFactionId:useMapStore.getState().campaign.playerFactionId,round:useMapStore.getState().campaign.round});setAppView('workspace')}}} onEditor={()=>{setMode('edit');setAppView('workspace')}} onBack={()=>setAppView('menu')} onStart={(factionId,fogEnabled,activeFactionIds,strategicDifficulty,rtsDifficulty)=>{newGame(factionId,fogEnabled,activeModId,activeFactionIds,strategicDifficulty,rtsDifficulty);void startSessionLog('campaign',{appVersion:GAME_VERSION,modId:activeModId,playerFactionId:factionId,fogEnabled,activeFactionIds,strategicDifficulty,rtsDifficulty},extractSaveGame(currentWorld(),null,activeModId??'default'));setSaveCompatible(true);setSaveCompatibilityReason(null);setAppView('workspace')}}/>
