...
const SETTLEMENT_LABELS: Record<SettlementType, string> = {
  village:'Деревня',city:'Город',fortress:'Крепость',capital:'Столица',port:'Порт',mine:'Шахта / кузница',farm:'Ферма / пастбище',wilderness:'Дикое владение',swamp:'Болото',forest:'Лес',mountains:'Горы',ruins:'Руины',crossroads:'Перекрёсток',ford:'Брод',pass:'Перевал',signal_tower:'Сигнальная башня',camp:'Лагерь',
}
const DOMAIN_ECONOMIC_TYPES:SettlementType[]=['capital','city','fortress','village','mine','farm','port','wilderness','swamp','forest','mountains']
const STRONGHOLD_ECONOMIC_TYPES:SettlementType[]=['fortress','ruins','crossroads','ford','pass','signal_tower','camp']
const SETTLEMENT_DEFAULTS: Record<SettlementType, { gold: number; materials: number; recruitmentSlots: number; reserveLimit: number }> = {
    ...
    <aside className="side-panel right-panel">
      ...
      <div className="inspector-body">
        ...
        <section className="inspector-section">
          ...
          <p className="field-help">Массовое назначение связывает всю выбранную область с одной опорной локацией и её владельцем.</p>
        </section>
    ...
    <aside className="side-panel right-panel">
      ...
      <div className="inspector-body">
        ...
        {!fogged && <section className="inspector-section">
          ...
          <label className="field">
            <span>Стратегический регион</span>
            <select value={cell.regionId ?? ''} disabled={readonly} onChange={(event) => patch({ regionId: event.target.value || null })}>
              <option value="">Без региона</option>
              {orderedRegions.map((region) => <option key={region.id} value={region.id}>{region.name}</option>)}
            </select>
          </label>
          <p className="field-help">Регион определяет владение территорией через свою опорную локацию. Проход армии сам по себе регион не захватывает.</p>
        </section>}

        <section className="inspector-section">
          <h3>Привязка к локации</h3>
          {boundLocations.length > 0 ? (
            ...
            <div className="nearest-location">
              <span>Ближайшая локация</span>
              <b>⌖ {nearestLocation?.name ?? 'Не определена'}</b>
              {!readonly && <small>Её идентификатор используется как контекст боя возле локации.</small>}
            </div>
          )}
        </section>
    ...
    <aside className="side-panel right-panel">
      ...
      <div className="inspector-body">
        ...
        <section className="inspector-section army-slots-section">
          ...
          <div className="army-slot-list">{army.heroSlots.map((slot, index) => { const hero = heroes.find((item) => item.id === slot.entityId); return <div className="army-slot-row hero" key={slot.slotId}><span className="slot-index">{index + 1}</span><span className="slot-portrait hero" style={hero?.portrait ? { backgroundImage: `url(${hero.portrait})` } : undefined}></span><div className="slot-identity"><b>{hero?.name ?? 'Неизвестный герой'}</b>{!readonly && <code>{slot.objectId}</code>}<small>Герой поддержки</small></div><span className="slot-power">Сила <b>{hero?.battlePower ?? 0}</b></span>{!readonly && <button type="button" onClick={() => updateArmy(army.id, { heroSlots: army.heroSlots.filter((_, i) => i !== index) })}>×</button>}{canManageReserve && stationedLocation && <button type="button" className="transfer-slot-button" title="Переместить в резерв локации" onClick={() => transferArmyToReserve(stationedLocation.id, army.id, slot.slotId)}>←</button>}</div>})}</div>
        ...
        <section className="inspector-section army-slots-section">
          ...
          <div className="army-slot-list">{army.unitSlots.map((slot, index) => { const unit = unitTypes.find((item) => item.id === slot.entityId); return <div className="army-slot-row unit" key={slot.slotId}><span className="slot-index">{index + 1}</span><span className="slot-portrait unit" style={unit?.portrait ? { backgroundImage: `url(${unit.portrait})` } : undefined}></span><div className="slot-identity"><b>{unit?.name ?? 'Неизвестный отряд'}</b>{!readonly && <code>{slot.objectId}</code>}<small>Боевой отряд</small></div><span className="slot-power">Сила <b>{unit?.battlePower ?? 0}</b></span>{!readonly && <button type="button" onClick={() => updateArmy(army.id, { unitSlots: army.unitSlots.filter((_, i) => i !== index) })}>×</button>}{canManageReserve && stationedLocation && <button type="button" className="transfer-slot-button" title="Переместить в резерв локации" onClick={() => transferArmyToReserve(stationedLocation.id, army.id, slot.slotId)}>←</button>}</div>})}</div>
...
export default function Inspector({ activeModId }: { activeModId:string }) {
      ...
      <aside className="side-panel right-panel">
        ...
        {hexEdit ? (
          ...
          <div className="inspector-empty">
            ...
            <b>Выберите объект</b>
            <p>Нажмите на локацию или включите тактический режим, чтобы исследовать логические гексы.</p>
          </div>
        )}
      </aside>
    )
  ...
  const locationRegion=regions.find((region)=>region.locationId===location.id)
    ...
    <aside className="side-panel right-panel">
      ...
      <div className="inspector-body">
        ...
        {!readonly ? <section className="inspector-section">
          ...
          <label className="field"><span>Экономический тип</span><select value={location.economicType} onChange={(event) => { const economicType = event.target.value as SettlementType; const defaults = SETTLEMENT_DEFAULTS[economicType]; updateLocation(location.id, { economicType, income: { gold: defaults.gold, materials: defaults.materials }, recruitmentSlots: defaults.recruitmentSlots, reserveLimit: defaults.reserveLimit }) }}>{(location.structuralType==='domain'?DOMAIN_ECONOMIC_TYPES:STRONGHOLD_ECONOMIC_TYPES).map((id)=><option key={id} value={id}>{SETTLEMENT_LABELS[id]}</option>)}</select></label>
          ...
          <header><div><small>{SETTLEMENT_LABELS[location.economicType]}</small><b>Экономика локации</b></div><span>+{location.income.gold} зол. · +{location.income.materials} мат.</span></header>
        ...
        <section className="inspector-section">
          ...
          {!readonly && <p className="field-help auto-region-help">Регион создаётся автоматически и всегда называется так же, как эта опорная локация.</p>}
          <label className="field">
          ...
          {!readonly && <div className="location-hex-card">
            <span className="hex-card-symbol">⬡</span>
            <div><small>Центральный гекс</small><b>{locationCellId}</b><i>{locationCell ? TERRAIN_BY_ID[locationCell.terrain].label : 'За пределами сетки'}</i></div>
            <button type="button" onClick={() => { selectHex(locationCellId); setViewMode('strategic') }}>Открыть</button>
          </div>}
          <div className="region-capture-card" style={{'--region-owner':regionOwner?.color??faction.color}as CSSProperties}><span>{location.structuralType==='domain'?'▧':'⬢'}</span><div><small>{location.structuralType==='domain'?'Контроль региона':'Самостоятельный гекс'}</small><b>{location.structuralType==='domain'?`${locationRegion?.name??location.name} · ${regionOwner?.label??'Нейтральный'}`:`${location.name} · ${faction.label}`}</b><p>{location.structuralType==='domain'?'Захват владения передаёт весь окружающий регион.':'Захват оплота передаёт только его единственный гекс.'}</p></div></div>
