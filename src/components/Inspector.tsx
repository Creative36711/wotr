...
import { armyCommandPointLimit, armyCommandPoints, armyMovementBreakdown, armyMovementCap, armyUnitSlotCap, buildBfmeArmyPayload, commanderDefinition, createCaptainCommander, createHeroCommander, createHeroSlot, createUnitSlot, factionArmyLimit, factionCaptainCount, factionCaptainLimit, generateUniqueCaptainName, reserveCommandPoints } from '../game/army'
...
function ArmyInspector({ army }: { army: Army }) {
  ...
  const canAddUnit = (unit: typeof unitTypes[number]) => commandPoints + (unit.commandPoints ?? 0) <= commandPointLimit
  const canAddHero = (hero: typeof heroes[number]) => commandPoints + (hero.commandPoints ?? 0) <= commandPointLimit
  ...
  const reserveCp = stationedLocation ? reserveCommandPoints(campaign.locationStates[stationedLocation.id]?.reserve ?? [], unitTypes, heroes) : 0
  const [transferTargetArmyId, setTransferTargetArmyId] = useState('')
  const transferTargets = mode === 'game' ? armies.filter((item) => item.id !== army.id && item.factionId === army.factionId && item.hexId === army.hexId) : []
    ...
    <aside className="side-panel right-panel">
      ...
      <div className="inspector-body">
        <section className="hex-summary-card army-summary-card">
          <div><span>Очки командования</span><b>{commandPoints}/{commandPointLimit}</b></div>
        ...
        <section className="inspector-section">
          ...
          <p className="field-help">Лимит ОК определяется назначенным командиром и меняется вместе с ним.</p>
          ...
          <p className="field-help">За каждые 5 отрядов армия получает −1 ОД. Армий фракции: {factionArmies}/{limit}. Лимит ОК этой армии: {commandPointLimit}.</p>
        </section>
        ...
        {mode === 'game' ? canFactionPlan(campaign, factions, army.factionId) ? <section className="army-game-help planning"><b>Управление в фазе планирования</b><p>{stationedLocation ? `Армия находится во владении/оплоте «${stationedLocation.name}». Стрелка ← переносит отряд в резерв.` : 'Для пополнения и расформирования армия должна находиться на гексе своего объекта карты.'}</p><p>ЛКМ по пустому гексу или врагу — приказ движения, после приказа выделение снимается. ЛКМ по своему объекту — выбор, Alt+ЛКМ — приказ на свой гекс. Отмена: кнопка выше, Delete или ПКМ.</p>{stationedLocation && <button type="button" className="disband-army-button" onClick={() => { if (window.confirm(`Расформировать «${army.name}»? Войска перейдут в резерв, лишние будут распущены.`)) disbandArmy(stationedLocation.id, army.id) }}>Расформировать армию</button>}</section> : isMovementPhase(campaign.phase) ? <section className={`army-game-help ${army.engaged ? 'engagement' : ''}`}><b>{army.engaged ? 'Армия связана боем' : army.commander ? 'Приказ движения' : 'Армия без командира'}</b><p>{army.engaged ? 'Обычное движение заблокировано. Армия может отойти сразу в ближайшую свою локацию. Потери зависят от расстояния, а на следующий ход армия будет деморализована.' : army.commander ? 'ЛКМ по пустому гексу или вражеской армии — приказ, после приказа выделение снимается; ЛКМ по своему объекту — выбор, Alt+ЛКМ — приказ на свой гекс. Отмена: кнопка выше, Delete или ПКМ. При входе во вражеский гекс все оставшиеся ОД будут потрачены.' : 'Армия без командира остаётся неподвижной.'}</p>{army.engaged && active && <button type="button" className="engagement-retreat-button" onClick={() => retreatEngagedArmy(army.id)}>Отступить из боя</button>}</section> : <section className="army-game-help"><b>Армия ожидает приказов</b><p>В фазах конфликтов и последствий движение и управление составом недоступны.</p></section> : <section className="inspector-actions single-action"><button type="button" className="danger-button" onClick={() => { if (window.confirm(`Удалить армию «${army.name}»?`)) removeArmy(army.id) }}>Удалить армию</button></section>}
      </div>
    </aside>
  )
}

export default function Inspector({ activeModId, activeMod, onModChange, appSettings }: { activeModId:string; activeMod: ModDefinition | null; onModChange: (definition: ModDefinition) => void; appSettings: AppSettings | null }) {
  const {language}=useI18n()
  ...
  const reservedCommandCp = reserveCommandPoints(locationState.reserve, unitTypes, heroes) + locationState.recruitmentQueue.reduce((total, item) => total + (unitTypes.find((unit) => unit.id === item.entityId)?.commandPoints ?? 0), 0)
    ...
    <aside className="side-panel right-panel">
      ...
      <div className="inspector-body">
        ...
        {!readonly ? <section className="inspector-section">
          ...
          <div className="coordinate-grid"><label className="field"><span>Слоты очереди</span><input type="number" min="0" max="10" value={location.recruitmentSlots} onChange={(event) => updateLocation(location.id, { recruitmentSlots: Number(event.target.value) })} /></label><label className="field"><span>Лимит ОК резерва</span><input type="number" min="0" max="10000" value={location.commandPointLimit} onChange={(event) => updateLocation(location.id, { commandPointLimit: Number(event.target.value) })} /></label></div>
          ...
          {!readonly&&<RtsCoordinatesTool location={location} activeMod={activeMod} appSettings={appSettings}/>}
          ...
          {canFactionPlan(campaign, factions, location.side) ? <>
            ...
            <div className="recruitment-queue-list">{locationState.recruitmentQueue.map((item) => { const unit = unitTypes.find((candidate) => candidate.id === item.entityId); return <div key={item.id}><span className="mini-unit-portrait" style={unit?.portrait ? { backgroundImage: `url(${unit.portrait})` } : undefined}></span><span className="recruitment-item-copy"><b>{unit?.name ?? 'Неизвестный отряд'}</b><small>{item.turnsLeft === 0 ? 'Ожидает место в резерве' : `Готов через: ${item.turnsLeft} ход.`} · {unitTypes.find((candidate) => candidate.id === item.entityId)?.commandPoints ?? 0} ОК</small></span><button type="button" aria-label="Отменить найм" title="Отменить найм и вернуть ресурсы" onClick={() => cancelRecruitment(location.id, item.id)}>×</button></div>})}{!locationState.recruitmentQueue.length && <p>Очередь пуста</p>}</div>
            <h4>Нанять</h4>
            <div className="recruitable-unit-list">{recruitableUnits.map((unit) => { const affordable = (treasury?.gold ?? 0) >= unit.recruitCost.gold && (treasury?.materials ?? 0) >= unit.recruitCost.materials && locationState.recruitmentQueue.length < location.recruitmentSlots && reservedCommandCp + (unit.commandPoints ?? 0) <= location.commandPointLimit; return <div key={unit.id}><span className="mini-unit-portrait" style={unit.portrait ? { backgroundImage: `url(${unit.portrait})` } : undefined}></span><span><b>{unit.name}</b><small>{unit.recruitCost.gold} зол. · {unit.recruitCost.materials} мат. · {unit.recruitTime} ход. · {unit.commandPoints ?? 0} ОК</small></span><button type="button" aria-label={`Нанять: ${unit.name}`} title={reservedCommandCp + (unit.commandPoints ?? 0) > location.commandPointLimit ? `Не хватает ОК локации (${reservedCommandCp}/${location.commandPointLimit} ОК)` : `Добавить «${unit.name}» в очередь`} disabled={!affordable} onClick={() => queueRecruitment(location.id, unit.id)}>＋</button></div>})}{!recruitableUnits.length && <p>В этом объекте нет доступных отрядов</p>}</div>
            <h4>Резерв · {locationState.reserve.reduce((total, slot) => total + (slot.kind === 'unit' ? (unitTypes.find((unit) => unit.id === slot.entityId)?.commandPoints ?? 0) : (heroes.find((hero) => hero.id === slot.entityId)?.commandPoints ?? 0)), 0)}/{location.commandPointLimit} ОК</h4>
            ...
            <div className="reserve-unit-list">{locationState.reserve.map((slot) => { const unit = slot.kind === 'unit' ? unitTypes.find((candidate) => candidate.id === slot.entityId) : null; const hero = slot.kind === 'hero' ? heroes.find((candidate) => candidate.id === slot.entityId) : null; const entity = unit ?? hero; const transformations = unit ? sortByText(unitTypes.filter((candidate) => candidate.transformationSourceUnitId === unit.id), (candidate) => candidate.name) : []; return <div key={slot.slotId} className={transformations.length ? 'has-transformations' : ''}><span className={`mini-unit-portrait ${slot.kind}`} style={entity?.portrait ? { backgroundImage: `url(${entity.portrait})` } : undefined}></span><span><b>{entity?.name ?? (slot.kind === 'hero' ? 'Неизвестный герой' : 'Неизвестный отряд')}</b><small>{slot.kind === 'hero' ? `Герой · ${hero?.commandPoints ?? 0} ОК` : `Содержание: ${unit?.upkeep ?? 0} · ${unit?.commandPoints ?? 0} ОК`}</small></span>{targetArmyId && (() => { const targetArmy = armies.find((item) => item.id === targetArmyId); if (!targetArmy) return null; const targetCp = armyCommandPoints(targetArmy, unitTypes, heroes); const targetLimit = armyCommandPointLimit(targetArmy, heroes, captains); if (slot.kind === 'unit') { const cp = unit?.commandPoints ?? 0; const fits = targetCp + cp <= targetLimit; return <button type="button" title={fits ? 'Передать в выбранную армию' : `В «${targetArmy.name}» не хватает ОК (${targetCp}/${targetLimit} ОК)`} disabled={!fits} onClick={() => transferReserveToArmy(location.id, targetArmyId, slot.slotId)}>→</button> } const heroActive = campaign.heroStates[slot.entityId]?.status === 'active'; const heroFits = heroActive && (targetArmy.commander?.kind === 'captain' || targetArmy.heroSlots.length < targetArmy.heroSlotLimit); return <button type="button" title={heroFits ? 'Передать в выбранную армию' : !heroActive ? 'Герой недоступен (ранен)' : 'Нет свободного слота героя'} disabled={!heroFits} onClick={() => transferReserveToArmy(location.id, targetArmyId, slot.slotId)}>→</button> })()}{transformations.length > 0 && <div className="reserve-transform-actions"><small>Преобразовать в боевой отряд</small>{transformations.map((target) => { const affordable = (treasury?.gold ?? 0) >= target.recruitCost.gold && (treasury?.materials ?? 0) >= target.recruitCost.materials; return <button type="button" key={target.id} disabled={!affordable} onClick={() => transformReserveUnit(location.id, slot.slotId, target.id)}><span>{target.name}</span><b>{target.recruitCost.gold ? `${target.recruitCost.gold} зол.` : 'Бесплатно'}</b></button> })}</div>}</div>})}{!locationState.reserve.length && <p>Резерв пуст</p>}</div>
