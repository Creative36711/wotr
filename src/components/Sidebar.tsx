...
function CampaignSidebar({onFocus}:SidebarProps) {
  ...
  const ownArmyRow = (army: typeof armies[number]) => {
    ...
    return <button type="button" key={army.id} className={`campaign-army-row ${selectedArmyId === army.id ? 'active' : ''} ${army.engaged ? 'engaged' : ''}`} onClick={() => {selectArmy(army.id);onFocus(army.id)}} onContextMenu={(event)=>{if(order){event.preventDefault();cancelArmyOrder(army.id)}}}><span className="campaign-army-flag" style={{ '--army-color': playerFaction.color } as CSSProperties}>{army.engaged ? '⚡' : '⚔'}</span><span><b>{army.name}</b><small>{commandPoints}/{commandPointLimit} ОК · сила {Math.round(armyPower(army))}</small><i>{order?`Приказ движения · ${order.cost} ОД (Delete или ПКМ — отменить)`:army.engaged?'Связана боем':army.status==='retreating'?'Деморализована · сила −20%':commanderName??'Нет командира'}</i></span><strong>{army.movementRemaining}/{movementCap}</strong></button>
  }
