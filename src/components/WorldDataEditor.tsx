          ...
          {tab === 'regions' && <>
            ...
            <div className="region-editor-list">{orderedRegions.map((region) => {
              ...
              return <article key={region.id} style={{ '--region-color': region.color || owner?.color || '#69747a' } as CSSProperties}>
                ...
                <label><span>Название</span><input value={localizedValue(region.name, region.nameTranslations, language)} onChange={(event) => { const localized = localizedTranslationsPatch(language, region.name, region.nameTranslations, event.target.value); updateRegion(region.id, { name: localized.canonical, nameTranslations: localized.translations }) }} /></label>
