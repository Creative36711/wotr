  ...
  return (
    <div className="database-backdrop" onPointerDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <section className="database-drawer">
        <header><div><span className="eyebrow">Редактор игровых данных</span><h2>Мир и кампания</h2>{translationStats.length > 0 && <p className="translation-stats">Translations: {translationStats.map((item) => <span key={item.locale}>{item.locale} {item.percent}% ({item.translated}/{item.total})</span>)}</p>}</div><button type="button" className="database-close" onClick={onClose} aria-label="Закрыть редактор данных"></button></header>
